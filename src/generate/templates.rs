...
//!
//! Generates configuration templates that instruct AI tools to use agentmap documentation.
...
This project uses **agentmap** for AI-optimized documentation.
...
| Task | Command |
|------|---------|
| Regenerate docs | `agentmap` |
| Fast update (changed only) | `agentmap --diff main` |
| Check if stale | `agentmap --check` |
| Force full regen | `agentmap --force` |
...
mod tests {
    ...
    fn test_generate_template_unified() {
        ...
        assert!(cursor.contains(".agentmap/INDEX.md"));
        assert!(cursor.contains(".agentmap/AGENT.md"));
        assert!(cursor.contains("## Reading Protocol"));
        assert!(cursor.contains("## Documentation Structure"));
        assert!(cursor.contains("## Commands"));
        assert!(cursor.contains("agentmap --check"));
        assert!(cursor.contains("agentmap --diff main"));
    }
